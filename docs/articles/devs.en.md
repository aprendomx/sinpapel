...
5. **SLAs with teeth** — maximum time per state with automatic actions: notify, escalate, reject, alert. A management command on cron plus an `sla_breached` signal to hang anything else on.
...
- **It's 0.x.** The public API is stable in practice but still beta; pin exact versions (`sinpapel==0.7.0`).
